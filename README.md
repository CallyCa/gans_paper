...
Next, clone this repository by opening a terminal and typing the following commands (do not type the first `$` on each line, it's just a convention to show that this is a terminal prompt, not something else like Python code):
    ...
    $ git clone https://github.com/callyca/autoencoders-gans.git
    $ cd autoencoders-gans

If you do not want to install git, you can instead download [main.zip](https://github.com/ageron/autoencoders-gans/archive/main.zip), unzip it, rename the resulting directory to `autoencoders-gans` and move it to your development directory.
...
Next, make sure you're in the `autoencoders-gans` directory and run the following command. It will create a new `conda` environment containing every library you will need to run all the notebooks (by default, the environment will be named `base-conda`, but you can choose another name using the `-n` option):
...
## Start Jupyter
You're almost there! You just need to register the `base-conda` conda environment to Jupyter. The notebooks in this project will default to the environment named `python3`, so it's best to register this environment using the name `python3` (if you prefer to use another name, you will have to select it in the "Kernel > Change kernel..." menu in Jupyter every time you open a notebook):
...
Then we'll delete this project's `base-conda` environment:

    $ conda activate base
    $ conda env remove -n base-conda
